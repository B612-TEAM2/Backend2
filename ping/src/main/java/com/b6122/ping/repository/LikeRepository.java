...
*/
    ...
    public void save(@Param("pid") Long pid,  @Param("uid") Long uid ) {
            TypedQuery<Like> query = em.createQuery("Insert INTO Like l(post.id, user.id) VALUES (:pid, :uid)", Like.class);
            query.setParameter("pid", pid);
            query.setParameter("uid", uid);
    }

    public void delete(@Param("pid") Long pid, @Param("uid") Long uid ) {
        TypedQuery<Like> query = em.createQuery("delete FROM Like l WHERE l.post.id =:pid And l.user.id = :uid", Like.class);
        query.setParameter("pid", pid);
        query.setParameter("uid", uid);
    }
