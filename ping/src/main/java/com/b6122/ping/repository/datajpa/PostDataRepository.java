...
public interface PostDataRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT * FROM post p WHERE p.scope = 'PUBLIC' AND ST_Distance_Sphere(POINT(p.longitude, p.latitude), POINT(:longitude, :latitude)) <= 2000 ORDER BY p.created_date", nativeQuery = true)
